def inject_mobile_cards():
    filepath = 'src/pages/StudentPage.jsx'
    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()

    # Find the start and end lines for Edit Mode div
    edit_start_idx = -1
    edit_end_idx = -1
    view_start_idx = -1
    view_end_idx = -1
    
    # Locate Household Composition section
    for i, line in enumerate(lines):
        if 'Household Composition' in line:
            start_search = i
            break
            
    for i in range(start_search, start_search + 500):
        if '{editMode ? (' in lines[i]:
            edit_start_idx = i + 1
            break
            
    # We know edit_start_idx is `<div className="overflow-x-auto pb-2">`
    # Now find the matching closing `</div>` which precedes `) : (`
    brace_count = 0
    for i in range(edit_start_idx, len(lines)):
        if ') : (' in lines[i] and 'overflow-x-auto' in lines[i+1]:
            edit_end_idx = i - 1
            view_start_idx = i + 1
            break
            
    # Find the end of view mode
    for i in range(view_start_idx, len(lines)):
        if ')}' in lines[i] and '</div>' in lines[i-1]:
            view_end_idx = i - 1
            break

    print(f"Edit mode: {edit_start_idx} to {edit_end_idx}")
    print(f"View mode: {view_start_idx} to {view_end_idx}")

    mobile_cards_edit = """                              </div>
    ...
    if edit_start_idx != -1 and edit_end_idx != -1 and view_start_idx != -1 and view_end_idx != -1:
        # Perform replacements from bottom to top to preserve line indices
        lines[view_end_idx] = mobile_cards_view
        lines[view_start_idx] = '                            <>\n                              <div className="hidden md:block overflow-x-auto">\n'
        
        lines[edit_end_idx] = mobile_cards_edit
        lines[edit_start_idx] = '                            <>\n                              <div className="hidden md:block overflow-x-auto pb-2">\n'
        
        with open(filepath, 'w', encoding='utf-8') as f:
            f.writelines(lines)
        print("Successfully injected mobile cards into StudentPage.jsx")
    else:
